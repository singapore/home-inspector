...
casper.on('page.error', function(msg, trace) {
  this.echo('Error:    ' + JSON.stringify(msg), 'ERROR');
...
function crawl(link) {
  ...
  this.wait(waitVal);
  this.then(function() {
    var success = this.currentHTTPStatus === OK || this.currentHTTPStatus === unchanged;
    if (!success) {
      console.log('thiscurrentHTTPStatus = ' + this.currentHTTPStatus);
    }
    return casper.test.assert(success, 'Page is OK');
  });
  this.then(function() {
  ...
  this.then(function() {
    ...
    casper.each(failedResources, function(self, resource) {
      console.log('failedResource: ' + JSON.stringify(resource));
